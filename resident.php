...
<body>
            ...
            <div class="container p-5">
                ...
                <form action="resident.php" method="post">
                    <div class="row g-3 d-flex mb-4">
                        <div class="col-auto">
                            <h1 class="h3 page-title">Residents</h1>
                        </div>
                        <div class="col-auto">
                            <input
                                type="search"
                ...
                if (isset($_POST['btnsearch'])) {
                    ...
                    // professor‑style concatenation, no real_escape_string
                    $selectsql = "SELECT * FROM resident WHERE 
                resident_id LIKE '%"   . $searchInput . "%' OR 
                resident_name LIKE '%" . $searchInput . "%' OR 
                date_of_birth LIKE '%" . $searchInput . "%' OR 
                gender LIKE '%"        . $searchInput . "%' OR 
                contact_information LIKE '%" . $searchInput . "%' OR 
                address LIKE '%"       . $searchInput . "%'";
                } else {
                    //if search btn is not click display all
                    $selectsql = "SELECT * FROM resident";
                }
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" id="projectTable">
                        ...
                        <tbody>
                            <?php
                            //for each to use for displaying arrays
                            if ($result && $result->num_rows > 0) {
                                foreach ($result as $field) {
                                    //yung 2d array sa 2d, another array
                                    echo "<tr>";
                                    echo "<td>" . $field['resident_id']          . "</td>";
                                    echo "<td>" . $field['resident_name']        . "</td>";
                                    echo "<td>" . $field['date_of_birth']        . "</td>";
    ...
    <!--
  <script>
    const searchBox   = document.querySelector('input[name="searchInput"]');
